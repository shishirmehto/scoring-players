...
def load_bootstrap_static(
    bootstrap_path: Optional[str], bootstrap_url: Optional[str]
) -> dict:
    ...
    if bootstrap_url:
        ...
        r = requests.get(
            bootstrap_url, headers={"User-Agent": "Mozilla/5.0"}, timeout=30
        )
        r.raise_for_status()
        return r.json()
...
def build_player_table(
    bootstrap: dict, merged_gw: pd.DataFrame, players_raw: pd.DataFrame
) -> pd.DataFrame:
    ...
    if "element" not in mgw.columns:
        key = (
            "element"
            if "element" in mgw.columns
            else ("id" if "id" in mgw.columns else None)
        )
        if key is None:
            raise ValueError(
                "merged_gw missing 'element' or 'id' for player id mapping"
            )
...
def score_row(row: pd.Series) -> float:
    ...
    pen = availability_penalty(
        row.get("status"), row.get("chance_of_playing_next_round")
    )
...
def pick_squad(df_scored: pd.DataFrame, budget: float = 100.0) -> pd.DataFrame:
    pool = df_scored[
        ["id", "web_name", "team_name", "team", "position", "price", "score"]
    ].copy()
    ...
    if pulp is not None:
        ...
        x = {
            i: pulp.LpVariable(f"x_{int(pid)}", lowBound=0, upBound=1, cat="Binary")
            for i, pid in enumerate(pool["id"])
        }
        prob += pulp.lpSum([pool.iloc[i]["score"] * x[i] for i in range(len(pool))])
        prob += (
            pulp.lpSum([pool.iloc[i]["price"] * x[i] for i in range(len(pool))])
            <= budget
        )
    ...
    for _, row in pool.iterrows():
        ...
        if counts[pos] >= quotas[pos]:
            continue
        tm = row["team_name"]
        if team_counts.get(tm, 0) >= 3:
            continue
        if row["price"] > money + 1e-6:
            continue
        squad.append(row)
        counts[pos] += 1
...
def main():
    ...
    mapping = {
        "GK": "top5_goalkeepers.csv",
        "DEF": "top5_defenders.csv",
        "MID": "top5_midfielders.csv",
        "FWD": "top5_forwards.csv",
    }
    ...
    if not squad.empty:
        ...
        print(
            squad[["position", "web_name", "team_name", "price", "score"]].to_string(
                index=False
            )
        )
    else:
